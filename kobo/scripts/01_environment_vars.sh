...
echo "HOME_VAGRANT set to "  $HOME_VAGRANT
# Directories.
if [ -d /home/vagrant ]; then
...
kobo_workon () {
	. $HOME_VAGRANT/.virtualenvs/$1/bin/activate && . $HOME_VAGRANT/.virtualenvs/$1/bin/postactivate
}
kobo_mkvirtualenv () {
	virtualenv $HOME_VAGRANT/.virtualenvs/$1 --system-site-packages
	touch $HOME_VAGRANT/.virtualenvs/$1/bin/postactivate
	kobo_workon $1
}
