...
[ -f "$HOME_VAGRANT/boot_launch_servers.log" ] && { mv "$HOME_VAGRANT/boot_launch_servers.log" $V_L; }
...
if [ -f "$V_S/run_kobocat.bash" ]; then
        echo "Starting kobocat logs at $V_L/kobocat.log"
	bash "$V_S/run_kobocat.bash" >> "$V_L/kobocat.log" 2>&1 &
fi

if [ -f "$V_S/run_koboform.bash" ]; then
        echo "Starting koboforn logs at $V_L/kobocat.log"
